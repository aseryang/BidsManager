...
LineChartFrame::LineChartFrame(const char* chartTitle,
	const char* yAxisTitle,
	const char* yAxisDataTitle,
	DoubleArray& timeStamps,
	DoubleArray& datas,
	QWidget *parent) :m_chartTitle(chartTitle),
	m_yAxisTitle(yAxisTitle),
	m_yAxisDataTitle(yAxisDataTitle),
	...
	QFrame(parent)
{
	...
	// Pointer push button
	QPushButton *pointerPB = new QPushButton(QIcon(":/pointer.png"), "鼠标", frame);
	...
	// Zoom In push button
	QPushButton *zoomInPB = new QPushButton(QIcon(":/zoomin.png"), "放大", frame);
	...
	QPushButton *zoomOutPB = new QPushButton(QIcon(":/zoomout.png"), "缩小", frame);
	...
	// Start Date control
	(new QLabel("起始日期", frame))->setGeometry(6, 230, 112, 18);
	...
	// Duration control
	(new QLabel("结束日期", frame))->setGeometry(6, 284, 112, 18);
...
LineChartFrame::~LineChartFrame()
{
	...
	if (m_timeStamps.data)
	{
		delete[] m_timeStamps.data;
	}
	if (m_dataSeriesA.data)
	{
		delete m_dataSeriesA.data;
	}
}
...
void LineChartFrame::loadData()
{
	...
	for (int i = 0; i < 8; ++i)
	{
		dataarray[i] = Chart::chartTime(2007, 1, 1) + 86400;
	}
	dataarray[8] = Chart::chartTime(2007, 1, 1) + 86400 * 2;
	dataarray[9] = Chart::chartTime(2007, 1, 1) + 86400 * 3;
	DoubleArray da(dataarray, 10);
	m_timeStamps = da;

	m_dataSeriesA = m_ranSeries->getSeries(10, 150, -10, 10);
...
// Initialize the QChartViewer
//
void LineChartFrame::initChartViewer(QChartViewer *viewer)
{
	...
	viewer->setZoomInWidthLimit(1.0 / m_timeStamps.len);
}
...
//
void LineChartFrame::onViewPortChanged()
{
...
//
void LineChartFrame::updateControls(QChartViewer *viewer)
{
...
//
void LineChartFrame::drawChart(QChartViewer *viewer)
{
	...
	c->setPlotArea(55, 50, c->getWidth() - 90, c->getHeight() - 90, c->linearGradientColor(0, 55, 0,
		c->getHeight() - 35, 0xf0f6ff, 0xa0c0ff), -1, Chart::Transparent, 0xffffff, 0xffffff);

	// As the data can lie outside the plotarea in a zoomed chart, we need enable clipping.
	c->setClipping();

	// Add a title to the chart using 18 pts Times New Roman Bold Italic font timesbi.ttf
	c->addTitle(m_chartTitle, "simsun.ttc", 11);

	// Add a legend box at (55, 30) using horizontal layout. Use 8pts Arial Bold as font. Set the
	// background and border color to Transparent and use line style legend key.
	/*LegendBox *b = c->addLegend(55, 55, false, "arialbd.ttf", 8);
	...
	c->yAxis()->setTitle(m_yAxisTitle, "simsun.ttc", 10);
	...
	layer->addDataSet(viewPortDataSeriesA, 0xff3333, m_yAxisDataTitle);
...
//
void LineChartFrame::onMouseUsageChanged(int mouseUsage)
{
...
//
void LineChartFrame::onStartDateChanged(QDateTime date)
{
...
//
void LineChartFrame::onEndDateChanged(QDateTime date)
{
	if (!m_ChartViewer->isInViewPortChangedEvent())
	{
		// The updated view port width
...
//
void LineChartFrame::onHScrollBarChanged(int value)
{
	if (!m_ChartViewer->isInViewPortChangedEvent())
	{
...
//
void LineChartFrame::onMouseWheelChart(QWheelEvent *event)
{
...
//
void LineChartFrame::onMouseMovePlotArea(QMouseEvent *)
{
...
//
void LineChartFrame::trackLineLabel(XYChart *c, int mouseX)
{
